...
const TILE_COLORS = ["#6366f1", "#8b5cf6", "#ec4899", "#f97316", "#eab308", "#22c55e", "#06b6d4", "#14b8a6"]
...
export const ProjectsScreen: FC<HomeStackScreenProps<"Projects">> = ({ navigation }) => {
  ...
  const { data, loading, refreshing, refresh } = useCachedList<Project>(
    activeOrg ? `projects:${activeOrg.id}` : null,
    fetcher,
  )
  ...
  return (
    <Screen preset="fixed">
      <FlatList
        data={data}
        keyExtractor={(p) => p.id}
        contentContainerStyle={data.length === 0 ? $flex : undefined}
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={colors.tint} />
        }
        ListEmptyComponent={
          <EmptyState icon="folder-open-outline" title="No projects" caption="Projects you can access will appear here." />
        }
        renderItem={({ item }) => (
          <Pressable
            onPress={() => navigation.navigate("ProjectDetail", { projectId: item.id, title: item.name })}
            style={({ pressed }) => [
              $row,
              { backgroundColor: pressed ? colors.muted : colors.background, borderBottomColor: colors.separator },
            ]}
          >
            <View style={[$tile, { backgroundColor: tileColor(item.key || item.name), borderRadius: radius.md }]}>
              <Text
                text={(item.key || item.name).charAt(0).toUpperCase()}
                style={{ color: "#FFFFFF", fontFamily: typography.display.bold, fontSize: 16 }}
              />
            </View>
            <View style={$grow}>
              ...
              <Text
                text={item.key}
                style={{ color: colors.textDim, fontFamily: typography.code.normal, fontSize: 11 }}
              />
            </View>
          </Pressable>
        )}
