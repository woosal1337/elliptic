...
description: >-
  ...
  tools, 155 of them). Use this skill whenever you read or write Elliptic
  projects, tasks, triage, meetings, notes, comments, calendar events,
  automations, AI agents, or org settings — it explains the domain model, the
...
markdown notes, a calendar, automations, and the AI agents and people doing the
work. Its API embeds a first-party MCP server that exposes the whole workspace:
...
├── Projects  (key like ENG; board, members, intake/triage, artifacts=links)
...
├── Meetings  (transcript segments → AI summaries, recipes, share links)
├── Calendar events  (team-visible or personal)
├── Vocabulary  (org glossary injected into every meeting AI call)
├── Automations (triage/status triggers → label/assign/route/set_priority)
└── AI users + BYOK provider keys + AI runs (the org's own agent roster & spend)
```
...
3. **Deletes are two-phase.** Destructive tools take `confirm: false` by
   ...
   act immediately): `delete_calendar_event`, `remove_project_member`,
   `remove_team_member`, `remove_task_relation`, `remove_project_artifact`,
   `revoke_invite`, `decline_triage_task`, `update_member_role`.
...
6. **Pass `idempotency_key` on creates you might retry.** Thirteen create
   ...
   `create_calendar_event`, `create_team`, `create_view`, `create_term`,
...
7. **Always pass a sane `limit`.** The MCP path has **no server-side cap**
   (the REST API caps at 200; MCP does not). `get_task_board`,
   `list_calendar_events`, `list_triage` and several list-all tools return
   everything unpaginated — fine for normal workspaces, heavy for big ones.
...
9. **Respect admin intent even where MCP doesn't enforce it.** A few
   ...
   templates, vocabulary, AI users/keys, `create_project`, `create_team`,
   `add_project_member`). Treat those as admin actions: don't perform them for
   a non-admin user without their explicit ask.
10. **AI-powered tools spend the org's own key.** `summarize_meeting`,
    `ask_meeting`, `meetings_chat`, `run_meeting_recipe` require the org to
    have AI enabled and a BYOK provider key; each call writes an auditable AI
    run. `search`, `get_event_brief`, `suggest_meeting_project`,
    `list_meeting_chapters` are deterministic and free — prefer them when they
    suffice.

## What you can do, at a glance (155 tools)
...
| Meetings | 17 | transcripts, summaries, chapters | create/import, summarize, ask, cross-meeting chat, recipes, share links |
| Agents (AI users & keys) | 12 | roster, runs, masked keys | define personas, pause, budget, BYOK key CRUD |
...
| Calendar | 6 | window queries, event briefs | create/update/delete events |
| Notifications | 6 | inbox, unread count | read/archive/snooze |
| Meeting templates & recipes | 6 | list both | create/update/delete templates, save recipes |
...
| Triage | 3 | queue | accept / decline |
| Integrations (Slack) | 3 | status, channels | post meeting summary |
...
cycles/sprints, initiatives, milestones, releases, retrospectives, worklogs,
dashboards, favorites, approvals, PQL queries, project templates, triage
snooze/mark-duplicate, **comment/note attachment upload** (those upload through
the web app — Drive documents, by contrast, DO upload over MCP), Slack
...
2. `brain_open_threads` → your open assigned/created tasks + the triage queue.
...
Find work → do work: `list_triage` to route intake (`accept_triage_task` /
`decline_triage_task` with a reason), `get_task_board(project_id)` for the
board, `list_my_tasks(filter="assigned")` for your plate, then
...
`summarize_meeting` for a cited summary, then `create_tasks_batch(titles,
source_meeting_id=…)` to turn action items into tracked tasks — provenance
makes the done-loop notify attendees. `meetings_chat` answers questions across
...
| `No AI provider key configured for this organization` | AI tools need a BYOK key (`list_ai_keys` to check; an admin adds one) |
...
- **[references/domain.md](references/domain.md)** — entities and enums,
  statuses and transition rules, triage mechanics, roles and permissions,
...
- **[references/recipes.md](references/recipes.md)** — step-by-step playbooks:
  catch-up, triage sweep, meeting-to-tasks, weekly report, filing notes, filing
  Drive documents, Slack posting, multi-org hygiene.
